...
export default function HomeScreen() {
  ...
  const pingLocation = async () => {
    ...
    try {
      ...
      // Persist last_seen and location to profile only if profile exists (avoid creating incomplete rows)
      try {
        const { updateProfileIfExists } = await import('../../services/profileService');
        const point = `POINT(${longitude} ${latitude})`;
        const updated = await updateProfileIfExists(user, { last_seen: new Date().toISOString(), location: point } as any);
